...
def level_7():
    ...
    print ('시간 : 7시 50분')
    print (' ')
    print ('애미가 존나 깨워서 일어났는데 어 시발 지각각이다!')
    print ('애미 : 너 잘밤에 뭔짓했냐')
    print (' ')
    print ('1 : 아가리 싸물고 있는다')
    print ('2 : 그냥 잤는데요')
    while(1):
        inp = input ('선택 : ')
        if inp == '1':
            print ('애미 : 그래 기분 안좋으면 그럴 수도 있지')
            print ('와 시발 개쫄렸다;; 근데 이걸 봐주네;;')
            print (' ')
            gameover_clear()
        elif inp == '2':
            print ('애미 : 수상하다? 콤퓨타 사용기록 보자')
            print ('애미 : (콤퓨타 사용기록을 보고 반 담탱이한테 전화) 아 예 선생님 얘가 오늘 좀 아파서 학교를 못갈것 같아요 아 네네 예')
            print ('애미 : 이리와 넌 좀 진지하게 상담 좀 해야겠다')
            exit('')
        else:
            print ('잘못된 선택입니다')
            continue
